...
(function($, document) {
	...
	var domBuffer = '<div class="mui-dtpicker" data-type="datetime">\
		...
		<div class="mui-dtpicker-title"><h5 data-id="title-y">年</h5><h5 data-id="title-m">月</h5><h5 data-id="title-d">日</h5><h5 data-id="title-h">时</h5><h5 data-id="title-i">分</h5><h5 data-id="title-w">周</h5></div>\
		<div class="mui-dtpicker-body">\
			...
			<div data-id="picker-w" class="mui-picker">\
				<div class="mui-picker-inner">\
					<div class="mui-pciker-rule mui-pciker-rule-ft"></div>\
					<ul class="mui-pciker-list">\
					</ul>\
	...
	var DtPicker = $.DtPicker = $.Class.extend({
		init: function(options) {
			...
			ui.y.addEventListener('change', function(e) { //目前的change事件容易导致级联触发
				...
				if(self.options.beginWeek || self.options.endWeek) {
					self._createWeek();
				}
			}, false);
		...
		getSelected: function() {
			...
			switch(type) {
				...
				case 'hour':
					...
					break;
				case 'week':
					selected.value = selected.y.value + '/' + selected.m.value + '-' + selected.w.value;
					selected.text = selected.y.text + '/' + selected.m.text + '-' + selected.w.text;
					break;
			}
			return selected;
		},
		setSelectedValue: function(value) {
			...
			if(self.options.type == "week") {
				setTimeout(function() {
					ui.w.picker.setSelectedValue(parsedValue.w, 0);
				}, 100);
			}
		},
		...
		getWeekNum: function(year, month) {
		...
		_isBeginWeek: function() {
			return this._isBeginMonth() && this.options.beginWeek === parseInt(this.ui.w.picker.getSelectedValue());
		},
		...
		_isEndWeek: function() {
			return this._isEndMonth() && this.options.endWeek === parseInt(this.ui.w.picker.getSelectedValue());
		},
		...
		_createWeek: function(current) {
			...
			//生成日列表
			var wArray = [];
			if(options.customData.w) {
				dArray = options.customData.w;
			} else {
				var w = self._isBeginMonth() ? options.beginWeek : 1;
				var maxWeek = self._isEndMonth() ? options.endWeek : self.getWeekNum(parseInt(this.ui.y.picker.getSelectedValue()), parseInt(this.ui.m.picker.getSelectedValue()));
				for(; w <= maxWeek; w++) {
					var val = self._fill(w);
					wArray.push({
						text: val,
						value: val
					});
		...
		_parseValue: function(value) {
			...
			if(value) {
				...
				var parts = value.replace(":", "-").replace(" ", "-").replace(new RegExp('/', 'g'), '-').split("-");
				rs.y = parts[0];
				rs.m = parts[1];
				rs.d = parts[2];
				rs.h = parts[3];
				...
				rs.h = now.getHours();
				rs.i = now.getMinutes();
				//console.log(self.getWeekNum(rs.y, rs.m));
				//rs.w = 4; //self.getWeekNum(rs.y, rs.m);
				...
				rs.w = getMonthWeek(rs.y, rs.m, rs.d)
			}
			return rs;
		},
		_create: function(options) {
			var self = this;
			options = options || {};
			options.labels = options.labels || ['年', '月', '日', '时', '分', '周'];
