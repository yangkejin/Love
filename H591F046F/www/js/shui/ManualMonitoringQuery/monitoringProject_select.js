...
var datas = new Datas();
datas.GetDetectionItems(function(DBData) {
	if (DBData) {
...
function initData(index) {
	for (var i = 0; i < app.ExtensionData.length; i++) {
		...
		if (i == index) {
			dic.isSelect = true;
			app.Sections = app.ExtensionData[index].Items;
		} else {
			dic.isSelect = false;
		}
	} 
...
document.getElementById("none_v").addEventListener("click", function() {
	mui.plusReady(function() {
		mui.fire(plus.webview.getWebviewById("trend_query.html"), 'monitoringProject_select_back', app.selectData);
	})
	mui.back();
});
